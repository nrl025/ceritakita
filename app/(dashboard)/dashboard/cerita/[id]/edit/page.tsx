...
import { ArrowLeft, X, Image as ImageIcon, Save } from 'lucide-react';
...
export default function EditCeritaPage() {
  ...
  const fetchStory = async () => {
    try {
      ...
      const story = data.story; // API returns { story: {...} }
      ...
      setIsInteractive(story.isInteractive || false);
      setExistingThumbnail(story.thumbnail || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (thumbnail) {
        ...
        if (!uploadRes.ok) throw new Error('Gagal mengupload thumbnail');
      ...
      if (!res.ok) throw new Error('Gagal memperbarui cerita');
    ...
    <div className="max-w-4xl mx-auto space-y-4 md:space-y-6">
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl md:text-3xl font-bold text-black">Edit Cerita</h1>
          <p className="text-sm md:text-base text-gray-600 mt-1">Perbarui cerita Anda</p>
        </div>
      </div>
